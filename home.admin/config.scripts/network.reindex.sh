...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
 ...
 echo "There are two ways to re-index - for details see: https://glcoin.stackexchange.com/a/60711"
...
if [ "$1" = "reindex" ] || [ "$1" = "reindex-chainstate" ]; then
  ...
  sudo -u glcoin /usr/local/bin/${network}d ${netparam}-daemon -blockfilterindex=0 -${action} -conf=/mnt/hdd/app-data/${network}/${network}.conf -datadir=/mnt/hdd/app-storage/${network} 1>&2
