...
configure_default_torrc(){
  ...
  echo "## raspiblesk torrc (anchor, do not remove this line)
...
if [ "${action}" = "enable" ]; then
  ...
  if [ $(sudo grep -c "raspiblesk" ${torrc}) -lt 1 ]; then
  ...
  echo "# DO NOT EDIT! This file is generated by raspiblesk and will be overwritten
[Service]
ReadWriteDirectories=-${tor_data_dir}
[Unit]
After=network.target nss-lookup.target mnt-hdd.mount
" | sudo tee /etc/systemd/system/tor@default.service.d/raspiblesk.conf

  # fix apparmor - https://github.com/rootzoll/raspiblesk/issues/2531
