...
def main():
    ...
    description = """RaspiBlesk Config Check"""
    ...
    if os.path.exists(rb_cfg.abs_path):
        try:
            ...
            rb_cfg_valid = True
            if not args.quiet:
                print("RaspiBlesk Config: \tOK")
        except Exception as err:
            if not args.quiet:
                print("RaspiBlesk Config: \tERROR")
                log.warning(err)
                print("# Use command to fix: nano /mnt/hdd/app-data/raspiblesk.conf")
                print("# CTRL+o to save / CRTL+x to exit / then reboot")

    else:
        if not args.quiet:
            print("RaspiBlesk Config: \tMISSING")
    ...
    if os.path.exists(rb_info.abs_path):
        try:
            ...
            rb_info_valid = True
            if not args.quiet:
                print("RaspiBlesk Info: \tOK")
        except Exception as err:
            if not args.quiet:
                print("RaspiBlesk Info: \tERROR")
                log.warning(err)

    else:
        if not args.quiet:
            print("RaspiBlesk Info: \tMISSING")
