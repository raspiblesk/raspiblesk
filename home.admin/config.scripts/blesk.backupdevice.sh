...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
 ...
 echo "# blesk.backupdevice.sh status"
 echo "# blesk.backupdevice.sh on [?DEVICEUUID|DEVICENAME]"
 echo "# blesk.backupdevice.sh off"
 echo "# blesk.backupdevice.sh mount"
 exit 1
fi

echo "### blesk.backupdevice.sh ###"
...
if [ "$1" = "on" ]; then
  ...
  if [ "${uuid}" == "" ]; then
    ...
    # get status data
    source <(sudo /home/admin/config.scripts/blesk.backupdevice.sh status)
...
THIS WILL DELETE ALL DATA ON THAT DEVICE!
  ...
  # change raspiblesk.conf
  /home/admin/config.scripts/blesk.conf.sh set localBackupDeviceUUID "${uuid}"
  echo "activated=1"

  # mount device (so that no reboot is needed)
  source <(sudo /home/admin/config.scripts/blesk.backupdevice.sh mount)
  ...
  if [ "${lightning}" == "lnd" ] || [ "${lnd}" == "on" ]; then
    # copy SCB over
    cp /mnt/hdd/app-data/lnd/data/chain/glcoin/mainnet/channel.backup /mnt/backup/channel.backup 1>&2
  fi
  if [ "${lightning}" == "cl" ] || [ "${cl}" == "on" ]; then
    # copy ER over
    cp /home/glcoin/.lightning/glcoin/emergency.recover /mnt/backup/emergency.recover 1>&2
  fi

  if [ ${userinteraction} -eq 1 ]; then
    if [ ${isMounted} -eq 0 ]; then
      /home/admin/config.scripts/blesk.conf.sh set localBackupDeviceUUID "off"
