...
if [ $# -lt 3 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
  ...
  echo "blesk.git-verify.sh [PGPsigner] [PGPpubkeyLink] [PGPpubkeyFingerprint]"
  echo "To use 'git verify-tag' add the 'tag':"
  echo "blesk.git-verify.sh [PGPsigner] [PGPpubkeyLink] [PGPpubkeyFingerprint] <tag>"
  exit 1
fi
...
# sudo -u btcrpcexplorer /home/admin/config.scripts/blesk.git-verify.sh \
...
wget -O /var/cache/raspiblesk/pgp_keys_${PGPsigner}.asc "${PGPpubkeyLink}"
...
if [ $? -ne 0 ]; then
  ...
  curl -o /var/cache/raspiblesk/pgp_keys_${PGPsigner}.asc "${PGPpubkeyLink}"
  if [ $? -ne 0 ]; then
...
gpg --import --import-options show-only /var/cache/raspiblesk/pgp_keys_${PGPsigner}.asc
fingerprint=$(gpg --show-keys --keyid-format LONG /var/cache/raspiblesk/pgp_keys_${PGPsigner}.asc 2>/dev/null | grep -Ec "${PGPpubkeyFingerprint}")
...
gpg --import /var/cache/raspiblesk/pgp_keys_${PGPsigner}.asc
rm /var/cache/raspiblesk/pgp_keys_${PGPsigner}.asc
