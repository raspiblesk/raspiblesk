...
# see: https://github.com/rootzoll/raspiblesk/issues/936

# get basic system information
# these are the same set of infos the WebGUI dialog/controler has
source /home/admin/raspiblesk.info </dev/null

# command info
if [ "$1" == "" ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
 echo "# tools for the boot drive / sd card"
 echo "# blesk.bootdrive.sh status"
 echo "# blesk.bootdrive.sh fsexpand"
 exit 1
fi
...
if [ "${action}" == "status" ]; then
    ...
    echo "fsexpanded=${fsexpanded}" # from raspiblesk.info
...
if [ "${action}" == "fsexpand" ]; then
    ...
    sudo sed -i "s/^fsexpanded=.*/fsexpanded=1/g" /home/admin/raspiblesk.info
