...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
 echo "# managing the RaspiBlesk data - import, export, backup."
 echo "# blesk.migration.sh [status|export|import|export-gui|import-gui]"
...
downloadUnix="scp -r 'glcoin@${localip}:${defaultDownloadPath}/raspiblesk-*.tar.gz' ./"
downloadWin="scp -r glcoin@${localip}:${defaultDownloadPath}/raspiblesk-*.tar.gz ."
uploadUnix="scp -r ./raspiblesk-*.tar.gz admin@${localip}:${defaultUploadPath}"
uploadWin="scp -r ./raspiblesk-*.tar.gz admin@${localip}:${defaultUploadPath}"

# check for a filename in the upload path
firstMigrationFile=$(ls -1 ${defaultUploadPath}/raspiblesk-*.tar.gz 2>/dev/null | head -n 1)
...
if [ "$1" = "status" ]; then
  echo "# RASPIBLESK Data Import & Export"
...
#########################
# EXPORT RaspiBlesk Data
#########################

if [ "$1" = "export" ]; then

  echo "# RASPIBLESK DATA --> EXPORT"
  ...
  # get name of RaspiBlesk from config (optional if exists)
  blitzname="-"
  ...
  # sudo tar -zcvf ${defaultDownloadPath}/raspiblesk-export-temp.tar.gz "${dataPath}" 1>~/.include.temp 2>/dev/null
  sudo tar -zcvf "${defaultDownloadPath}/raspiblesk-export-temp.tar.gz" -C "$(dirname "$dataPath")" "$(basename "$dataPath")" 1>~/.include.temp 2>/dev/null
  ...
  md5checksum=$(md5sum ${defaultDownloadPath}/raspiblesk-export-temp.tar.gz | head -n1 | cut -d " " -f1)
  echo "md5checksum=${md5checksum}"
  
  # get byte size
  bytesize=$(wc -c ${defaultDownloadPath}/raspiblesk-export-temp.tar.gz | cut -d " " -f 1)
  echo "bytesize=${bytesize}"

  # final renaming 
  name="raspiblesk${blitzname}${datestamp}-${md5checksum}.tar.gz"
  echo "exportpath='${defaultDownloadPath}'"
  echo "filename='${name}'"
  sudo mv ${defaultDownloadPath}/raspiblesk-export-temp.tar.gz ${defaultDownloadPath}/${name}
  sudo chown glcoin:glcoin ${defaultDownloadPath}/${name}
...
if [ "$1" = "export-gui" ]; then
  ...
  # make sure glcoin & lighning is stopped
  ...
  source <(sudo /home/admin/config.scripts/blesk.migration.sh export)
  ...
  echo 
  echo "Your Lightning node is now stopped. After download press ENTER to shutdown your raspiblesk."
...
if [ "$1" = "import-gui" ]; then
  ...
  echo "Go into the folder where you have stored your raspiblesk-*.tar.gz file and run:"
...
if [ "$1" = "import" ]; then
  ...
  # copy glcoin data backups back to original places (if part of backup before v1.12)
  if [ -d "/mnt/hdd/backup_glcoin" ]; then
    echo "# Copying back glcoin backup data .."
    sudo mkdir -p /mnt/app-data/glcoin
    sudo chown -R glcoin:glcoin /mnt/hdd/app-data/glcoin
    sudo cp /mnt/hdd/backup_glcoin/glcoin.conf /mnt/hdd/app-data/glcoin/glcoin.conf
    sudo cp /mnt/hdd/backup_glcoin/wallet.dat /mnt/hdd/app-data/glcoin/wallet.dat  2>/dev/null
    rm -rf /mnt/hdd/backup_glcoin
  fi
  ...
  if [ "${vm}" == "1" ]; then
    /home/admin/config.scripts/blesk.conf.sh set displayClass "headless"
  fi
