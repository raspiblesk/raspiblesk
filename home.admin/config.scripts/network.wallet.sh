...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
 echo "config script to switch the Glcoin Core wallet on or off"
...
# add disablewallet with default value (0) to glcoin.conf if missing
...
if [ "$1" = "1" ] || [ "$1" = "on" ]; then
  ...
  # glcoin config for wallet name & dir will be done by glcoin.check.sh prestart
