#!/bin/bash
# for reboot call: sudo /home/admin/config.scripts/blesk.shutdown.sh reboot
...
# glcoind
if [ "${network}" != "" ]; then
  # stopping glcoin (thru cli)
  echo "stop ${network}d (1) - please wait .."
  timeout 120 sudo -u glcoin ${network}-cli stop 2>/dev/null

  # stopping glcoind (thru systemd)
