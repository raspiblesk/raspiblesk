...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
 ...
 echo "blesk.ups.sh on apcusb"
 echo "blesk.ups.sh on x708"
 echo "blesk.ups.sh status"
 echo "blesk.ups.sh off"
 exit 1
fi
...
if [ "$1" = "1" ] || [ "$1" = "on" ]; then
  ...
  if [ "$2" = "apcusb" ]; then
    ...
    # give the RaspiBlesk a minimum of 15 min to shutdown
    ...
    sudo sed -i "s/^SHUTDOWN=.*/SHUTDOWN=\/home\/admin\/config.scripts\/blesk.shutdown.sh/g" /etc/apcupsd/apccontrol
    sudo sed -i "s/^WALL=.*/#WALL=wall/g" /etc/apcupsd/apccontrol
    sudo systemctl enable apcupsd
    sudo systemctl start apcupsd

    # set ups config value (in case of update)
    /home/admin/config.scripts/blesk.conf.sh set ups "apcusb"
    ...
    git clone https://github.com/HiLivin/x708blesk.git /home/admin/x708blitz
    ...
    # set ups config value (in case of update)
    /home/admin/config.scripts/blesk.conf.sh set ups "x708"
...
if [ "$1" = "0" ] || [ "$1" = "off" ]; then
  ...
  if [ "${ups}" = "apcusb" ]; then
    ...
    /home/admin/config.scripts/blesk.conf.sh set ups "off"
