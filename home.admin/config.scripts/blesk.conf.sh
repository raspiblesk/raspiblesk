...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ] || [ "$1" = "-help" ]; then
  echo "RaspiBlesk Config Edit - adds value to file & cache and creates entries if needed:"
  echo "blesk.conf.sh set [key] [value] [?conffile] <noquotes>"
  echo "blesk.conf.sh delete [key] [?conffile]"
  echo "blesk.conf.sh list-add [key] [value] [?conffile]"
  echo "blesk.conf.sh list-remove [key] [value] [?conffile]"
...
if [ "$1" = "set" ]; then
  ...
  raspibleskConfExists=$(ls ${configFile} 2>/dev/null | grep -c "${configFile}")
  if [ ${raspibleskConfExists} -eq 0 ]; then
    # shellcheck disable=SC2145
    echo "# blesk.conf.sh $@"
    exit 3
  fi
  ...
  do
     if [ "${value}" == "${valuestr}" ]; then
      echo "# blesk.conf.sh --> value(${valuestr}) already part of list with key(${keystr})"
      exit 1
     fi
  done
