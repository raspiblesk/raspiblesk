...
WhitepaperFilename="/glcoin.pdf"

# command info
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
  echo "# config script to download the Glcoin whitepaper to your Raspiblitz directly from the blockchain"
...
From the user home folder, call an editor 'nano glcoin.pdf' to enter the glcoin.txt or use 'less glcoin.txt' command, both to view the file in ASCII format.
" 13 78
  exit 0
fi
...
if [ "$1" = "1" ] || [ "$1" = "on" ]; then
    ...
    sudo -u glcoin glcoin-cli getblock 00000000000000ecbbff6bafb7efa2f7df05b227d5c73dca8f2635af32a2e949 0 |\
    tail -c+92167 |\
    ...
    /home/admin/config.scripts/blesk.conf.sh set whitepaper "on"
...
if [ "$1" = "0" ] || [ "$1" = "off" ]; then
  ...
  if [ ${isInstalled} -eq 1 ]; then
    ...
    echo ""
    # setting value in raspi blitz config
    /home/admin/config.scripts/blesk.conf.sh set whitepaper "off"
    rm $DownloadPath$WhitepaperFilename
    rm $DownloadPath/glcoin.txt
