...
# This is for developing on your RaspiBlesk.
...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ] || [ "$1" = "-help" ]; then
  ...
  echo "RaspiBlesk Sync Scripts"
  echo "blesk.github.sh info"
  echo "blesk.github.sh [-run|-install|-justinstall] branch [repo]"
  exit 1
fi
...
# gather info
activeGitHubUser=$(sudo -u admin cat /home/admin/raspiblesk/.git/config 2>/dev/null | grep "url = " | cut -d "=" -f2 | cut -d "/" -f4)
...
if [ ${#wantedGitHubUser} -gt 0 ]; then
  ...
  if [ "${activeGitHubUser}" = "${wantedGitHubUser}" ]; then
    ...
    repoExists=$(curl --header "X-GitHub-Api-Version:2022-11-28" -s https://api.github.com/repos/${wantedGitHubUser}/raspiblesk | jq -r '.name' | grep -c 'raspiblesk')
    ...
    git remote set-url origin https://github.com/${wantedGitHubUser}/raspiblesk.git
    activeGitHubUser=$(sudo -u admin cat /home/admin/raspiblesk/.git/config | grep "url = " | cut -d "=" -f2 | cut -d "/" -f4)
  fi
fi
...
if [ ${#wantedBranch} -gt 0 ]; then
  ...
  if [ "${wantedBranch}" = "${activeBranch}" ]; then
    ...
    if [ ${localBranch} -eq 0 ]; then
      ...
      branchExists=$(curl --header "X-GitHub-Api-Version:2022-11-28" -s https://api.github.com/repos/${activeGitHubUser}/raspiblesk/branches/${wantedBranch} | jq -r '.name' | grep -c ${wantedBranch})
...
echo "# *** SYNCING RASPIBLESK CODE WITH GITHUB ***"
echo "# This is for developing on your RaspiBlesk."
...
sudo rm -f *.sh
sudo -u admin cp /home/admin/raspiblesk/home.admin/.tmux.conf /home/admin
sudo -u admin cp /home/admin/raspiblesk/home.admin/*.* /home/admin 2>/dev/null
sudo -u admin chmod 755 *.sh
echo "# - asset directory"
sudo rm -rf assets
sudo -u admin cp -R /home/admin/raspiblesk/home.admin/assets /home/admin/assets
echo "# - config.scripts directory"
sudo rm -rf /home/admin/config.scripts
sudo -u admin cp -R /home/admin/raspiblesk/home.admin/config.scripts /home/admin/config.scripts 
...
sudo -u admin cp -R /home/admin/raspiblesk/home.admin/setup.scripts /home/admin/setup.scripts
