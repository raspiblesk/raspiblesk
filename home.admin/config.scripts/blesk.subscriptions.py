...
########################################################
# SSH Dialogs to manage Subscriptions on the RaspiBlesk
########################################################
...
You have no active or inactive subscriptions.
    ...
    d.set_background_title("RaspiBlesk Subscriptions")
    code, tag = d.menu(
...
{toraddress}
for the RaspiBlesk service: {service}
...
'''.format(initdate=selected_sub['time_created'],
    ...
    if code == "extra":
        os.system("clear")
        if selected_sub['type'] == "letsencrypt-v1":
            cmd = "python /home/admin/config.scripts/blesk.subscriptions.letsencrypt.py subscription-cancel {0}".format(
                selected_sub['id'])
            print("# running: {0}".format(cmd))
            os.system(cmd)
            time.sleep(2)
        elif selected_sub['type'] == "ip2tor-v1":
            cmd = "python /home/admin/config.scripts/blesk.subscriptions.ip2tor.py subscription-cancel {0}".format(
                selected_sub['id'])
            print("# running: {0}".format(cmd))
            os.system(cmd)
            time.sleep(2)
...
def main():
    ...
    if tag == "NEW2":
        ...
        os.system("clear")
        cmd = "python /home/admin/config.scripts/blesk.subscriptions.letsencrypt.py create-ssh-dialog"
        print("# running: {0}".format(cmd))
        os.system(cmd)
        sys.exit(0)
    ...
    run your RaspiBlesk behind TOR.
            ''', title="Info")
            sys.exit(0)
        ...
        d.set_background_title("RaspiBlesk Subscriptions")
        code, tag = d.menu(
            "\nChoose RaspiBlesk Service to create Bridge for:",
        ...
        # run creating a new IP2TOR subscription
        os.system("clear")
        cmd = "python /home/admin/config.scripts/blesk.subscriptions.ip2tor.py create-ssh-dialog {0} {1} {2}".format(
