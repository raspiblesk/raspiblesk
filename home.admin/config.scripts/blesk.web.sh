...
# TODO: later on this script will be run on build sdcard - make sure that the self-signed tls cert get created fresh on every new RaspiBlesk

source /mnt/hdd/app-data/raspiblesk.conf 2>/dev/null

# command info
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ] || [ "$1" = "-help" ]; then
  printf "Manage RaspiBlesk Web Interface(s)\n\n"
  printf "blesk.web.sh check \t\tprint operational nginx listen status (lsof)\n"
  printf "blesk.web.sh http-on \t\tturn on basic http & api\n"
  printf "blesk.web.sh https-on \t\tturn on https (needs hdd)\n"
  printf "blesk.web.sh off \t\tturn off\n"
  exit 1
fi
...
###################
  ...
  sudo tee /etc/systemd/system/nginx.service.d/raspiblesk.conf >/dev/null <<EOF
    # DO NOT EDIT! This file is generate by raspiblesk and will be overwritten
[Unit]
...
###################
  ...
  ### RaspiBlesk Webserver on HTTPS 443
