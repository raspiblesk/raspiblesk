...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ] || [ "$1" = "-help" ]; then
  echo "RaspiBlesk Error Handling"
  echo
  echo "blesk.error.sh [source-script-name] [fixed-short-code] [?detail-message] [?additional-debugdata] [?logfile]"
  echo
  exit 1
fi
...
if [ "${script}" == "" ]; then
  /home/admin/config.scripts/blesk.error.sh blesk.error.sh "error-missing-script" "missing any parameter" "$@"
  exit 1
fi
if [ "${shortcode}" == "" ]; then
  /home/admin/config.scripts/blesk.error.sh blesk.error.sh "error-missing-code" "script ${script} missing error code" "$@"
  exit 1
fi
...
if [ "${logfile}" !=  "" ]; then
  ...
  echo "${errorReport}" >> /home/admin/raspiblesk.log
fi
