...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
 echo "# use to prepare & check scp or web file upload to RaspiBlesk"
 echo "# blesk.upload.sh prepare-upload"
 echo "# blesk.upload.sh check-upload ?[scb|lnd-rescue|migration]"
 exit 0
fi
...
if [ -d "/mnt/hdd/temp" ]; then
  ...
  defaultUploadUser="glcoin"
else
  # fallback if no HDD is connected
  defaultUploadPath="/home/glcoin/temp/upload"
  defaultUploadUser="glcoin"
fi
...
if [ "${action}" == "check-upload" ]; then
  ...
  if [ "${type}" == "migration" ]; then
    ...
    # general filename check
    typeCount=$(sudo ls ${defaultUploadPath}/raspiblesk-*.tar.gz 2>/dev/null | grep -c 'raspiblesk')
    if [ "${typeCount}" != "1" ]; then
      sudo rm ${defaultUploadPath}/* 2>/dev/null
      echo "error='invalid'"
      echo "errorDetail='not raspiblesk-*.tar.gz'"
      exit 1
    fi
