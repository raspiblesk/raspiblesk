#!/usr/bin/env bash
# main repo: https://github.com/raspiblesk/raspiblesk-web
...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ] || [ "$1" = "-help" ]; then
  echo "Manage RaspiBlesk WebUI"
  echo "blesk.web.ui.sh info"
  echo "blesk.web.ui.sh on [GITHUBUSER] [REPO] [BRANCH] [?COMMITORTAG]"
  echo "blesk.web.ui.sh on DEFAULT"
  echo "blesk.web.ui.sh update"
  echo "blesk.web.ui.sh off"
  exit 0
fi
...
if [ "$1" = "info" ]; then
  # check if installed
  if ! cd /home/bleskapi/blitz_web 2>/dev/null; then
    echo "installed=0"
    exit 1
  fi
  echo "installed=1"
...
if [ "$1" = "1" ] || [ "$1" = "on" ]; then
  if [ "$2" == "DEFAULT" ]; then
    ...
    # copy build_sdcard.sh out of raspiblesk directory to not create "changes" in git
    ...
    # use dev branch when raspiblesk repo is n dev branch
...
###################
if [ "$1" = "update" ]; then
  webuiActive=$(sudo ls /home/bleskapi/blitz_web/README.md | grep -c "README")
  if [ "${webuiActive}" != "0" ]; then
    echo "# Update Web API"
    cd /home/bleskapi/blitz_web
    ...
    if [ "${oldCommit}" != "${newCommit}" ]; then
      ...
      sudo cp -r /home/bleskapi/blitz_web/build/* /var/www/public
...
if [ "$1" = "0" ] || [ "$1" = "off" ]; then
  ...
  sudo rm -rf /root/blitz_web /home/bleskapi/blitz_web /var/www/public/*
  exit 0
fi