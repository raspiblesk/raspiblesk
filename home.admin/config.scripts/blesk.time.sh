...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ] || [ "$1" = "-help" ]; then
  echo "RaspiBlesk Time Tools"
  echo
  echo "## Parameters #######"
  echo "choose-timezone  --> user can choose timezone from list and it gets stored to raspiblesk config"
  echo "set-by-config    --> resets the time on the RaspiBlesk based on the config"
  exit 1
fi
...
if [ "$1" = "choose-timezone" ]; then
  ...
  if [ -n "$choice" ]; then
    ...
    echo "# Saving timezone to raspiblesk config ..."
    /home/admin/config.scripts/blesk.conf.sh set "timezone" "$selected_timezone"
  else
...
if [ "$1" = "set-by-config" ]; then
  ...
  if [ ${#timezone} -eq 0 ]; then
    echo "# no timezone set in raspiblesk.conf ... keeping default timezone"
    exit 1
  fi
