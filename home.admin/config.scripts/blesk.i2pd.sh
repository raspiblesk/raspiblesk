...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
  ...
  echo "Usage:"
  echo "blesk.i2pd.sh install      -> Install i2pd"
  echo "blesk.i2pd.sh on           -> Switch on i2pd"
  echo "blesk.i2pd.sh off          -> Uninstall i2pd"
  echo "blesk.i2pd.sh addseednodes -> Add 21 randonly selected I2P seed nodes from: https://github.com/glcoin/glcoin/blob/master/contrib/seeds/nodes_main.txt"
  echo "blesk.i2pd.sh status       -> I2P related logs from glcoind, glcoin-cli -netinfo 4 and webconsole access"
  exit 1
fi
...
function glcoinI2Pstatus {
  echo "# I2P related logs from the glcoin debug log"
  echo "# Follow live with the command:"
  echo "sudo tail -n 1000 -f /mnt/hdd/app-data/glcoin/debug.log | grep i2p"
  echo
  sudo cat /mnt/hdd/app-data/glcoin/debug.log | grep i2p
  echo
  echo "# Running the command:"
  echo "glcoin-cli -netinfo 4"
  echo
  glcoin-cli -netinfo 4
  echo
...
if [ "$1" = "1" ] || [ "$1" = "on" ]; then
  ...
  if [ "${isInstalled}" != "0" ]; then
    ...
    /home/admin/config.scripts/blesk.i2pd.sh install
  fi
...
EOF
  ...
  /home/admin/config.scripts/blesk.conf.sh set i2pd "on"

  localip=$(hostname -I | awk '{print $1}')
  echo "# Config: /etc/i2pd/i2pd.conf"
  echo "# i2pd web console: ${localip}:7070"
  echo "# Monitor i2p in glcoind:"
  echo "sudo tail -n 100 /mnt/hdd/app-data/glcoin/debug.log | grep i2p"
  echo "glcoin-cli -netinfo 4"
...
if [ "$1" = "addseednodes" ]; then
  ...
  for i2pSeedNode in ${selectedNodes}; do
    ...
    echo "glcoin-cli addnode $i2pSeedNode onetry"
    sudo -u glcoin glcoin-cli addnode "$i2pSeedNode" "onetry"
  done
  
  echo
  echo "# Display glcoin-cli -netinfo 4"
  sudo -u glcoin glcoin-cli -netinfo 4
...
if [ "$1" = "0" ] || [ "$1" = "off" ]; then
  ...
  # setting value in raspiblesk.conf
  /home/admin/config.scripts/blesk.conf.sh set i2pd "off"
