...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ] || [ "$1" = "-help" ]; then
  echo "RaspiBlesk SSH tools"
  echo
  echo "## SSHD SERVICE #######"
  echo "blesk.ssh.sh renew       --> renew the sshd host certs & restarts sshd"
  echo "blesk.ssh.sh init        --> just creates sshd host certs"
  echo "blesk.ssh.sh clear       --> make sure old sshd host certs are cleared"
  echo "blesk.ssh.sh checkrepair --> check sshd & repair just in case"
  echo "blesk.ssh.sh backup      --> copy ssh keys to backup (if exist)"
  echo "blesk.ssh.sh sessions    --> count open sessions"
  echo "blesk.ssh.sh restore [?backup-root]"
  echo "                         --> restore ssh keys from backup (if exist)"
  echo 
  echo "## SSH ROOT USER #######"
  echo "blesk.ssh.sh root-get    --> return root user pubkey"
  echo "blesk.ssh.sh root-transfer [REMOTEUSER]@[REMOTESERVER]"
...
if [ "$1" = "checkrepair" ]; then
  ...
  if [ ${countKeyFiles} -lt 6 ]; then
    ...
    /home/admin/config.scripts/blesk.ssh.sh renew
  fi
