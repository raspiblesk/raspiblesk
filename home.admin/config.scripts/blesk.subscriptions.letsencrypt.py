...
if len(sys.argv) <= 1 or sys.argv[1] == "-h" or sys.argv[1] == "help":
    print("# manage letsencrypt HTTPS certificates for raspiblesk")
    print("# blesk.subscriptions.letsencrypt.py create-ssh-dialog")
    print("# blesk.subscriptions.letsencrypt.py subscriptions-list")
    print("# blesk.subscriptions.letsencrypt.py subscription-new <dyndns|ip> <duckdns> <domain> <token> [ip|tor|ip&tor]")
    print("# blesk.subscriptions.letsencrypt.py subscription-new <ip> <dynu> <domain> <clientid:secret> [ip|tor|ip&tor]")
    print("# blesk.subscriptions.letsencrypt.py subscription-detail <id>")
    print("# blesk.subscriptions.letsencrypt.py subscription-cancel <id>")
    print("# blesk.subscriptions.letsencrypt.py domain-by-ip <ip>")
    sys.exit(1)
...
def subscriptions_new(ip, dnsservice, domain, token, target):
    ...
    # https://github.com/rootzoll/raspiblesk/issues/1786
...
def menu_make_subscription():
    ...
    # ask user for which RaspiBlesk service the bridge should be used
    choices = []
...
Create one first and try again.
        ...
        # enter the static IP
        code, text = d.inputbox(
            "Enter the static public IP of this RaspiBlesk:",
            height=10, width=40, init="",
            title="Static IP")
        ip = text.strip()
        ip = ip.split(' ')[0]
...
to reach the service you wanted.
        ...
        # service flaky
        # https://github.com/rootzoll/raspiblesk/issues/1772
