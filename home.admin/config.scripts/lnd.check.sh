...
if [ "$1" == "prestart" ]; then
  ...
  chmod g+rx /mnt/hdd/app-data/lnd/data/chain/bitcoin/${CHAIN}
  ...
  passwordFile="/mnt/hdd/app-data/lnd/data/chain/bitcoin/${CHAIN}/password.info"
...
# check basic LND setup
  ...
  adminMacaroonExists=$(sudo ls /mnt/hdd/app-data/lnd/data/chain/bitcoin/${CHAIN}/admin.macaroon 2>/dev/null | grep -c 'admin.macaroon')
  if [ ${adminMacaroonExists} -gt 0 ]; then
    echo "macaroon=1"
  else
    echo "macaroon=0"
    echo "err='admin.macaroon is missing in /mnt/hdd/app-data/lnd/data/chain/bitcoin/${CHAIN}'"
  fi
  # check for admin macaroon exist (on SD card for admin)
  adminMacaroonExists=$(sudo ls /home/admin/.lnd/data/chain/bitcoin/${CHAIN}/admin.macaroon 2>/dev/null | grep -c 'admin.macaroon')
  if [ ${adminMacaroonExists} -gt 0 ]; then
    echo "macaroonCopy=1"
    # check if the same
    orgChecksum=$(sudo shasum -a 256 /mnt/hdd/app-data/lnd/data/chain/bitcoin/${CHAIN}/admin.macaroon 2>/dev/null | cut -d " " -f1)
    cpyChecksum=$(sudo shasum -a 256 /home/admin/.lnd/data/chain/bitcoin/${CHAIN}/admin.macaroon 2>/dev/null | cut -d " " -f1)
    if [ "${orgChecksum}" == "${cpyChecksum}" ]; then
  ...
  walletExists=$(sudo ls /mnt/hdd/app-data/lnd/data/chain/bitcoin/${CHAIN}/wallet.db 2>/dev/null | grep -c 'wallet.db')
