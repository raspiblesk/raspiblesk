...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
  ...
  echo "# blesk.display.sh image [path]"
  echo "# blesk.display.sh qr [datastring]"
  echo "# blesk.display.sh qr-console [datastring]"
  echo "# blesk.display.sh hide"
  echo
  echo "# sudo blesk.display.sh rotate [on|off]"
  echo "# sudo blesk.display.sh test-lcd-connect"
  echo "# sudo blesk.display.sh set-display [hdmi|lcd|headless]"
  echo "# sudo blesk.display.sh prepare-install"
  exit 1
fi

# 1. Parameter: lcd command
command=$1
echo "### blesk.display.sh $command"
...
if [ "${command}" == "qr" ]; then
  ...
  qrencode -l L -o /var/cache/raspiblesk/qr.png "${datastring}" > /dev/null
  # see https://github.com/rootzoll/raspiblesk/pull/1580
  if [ ${fb1Exists} -eq 1 ] ; then
    # LCD
    fbi -a -T 1 -d /dev/fb1 --noverbose /var/cache/raspiblesk/qr.png 2> /dev/null
  else
    # HDMI
    fbi -a -T 1 -d /dev/fb0 --noverbose /var/cache/raspiblesk/qr.png 2> /dev/null
  fi
  exit 0
fi
...
if [ "${command}" == "hide" ]; then
  killall -3 fbi
  rm /var/cache/raspiblesk/qr.png 2> /dev/null
  exit 0
fi

##################
# ROTATE
# see issue: https://github.com/rootzoll/raspiblesk/issues/681
###################
...
if [ "${command}" == "rotate" ]; then
  ...
  if [ "$2" = "1" ] || [ "$2" = "on" ]; then
    ...
    /home/admin/config.scripts/blesk.conf.sh set lcdrotate 1 1>/dev/null 2>/dev/null
...
EOF
    ...
    # update raspiblesk conf
    /home/admin/config.scripts/blesk.conf.sh set lcdrotate 0 1>/dev/null 2>/dev/null
    echo "OK - a restart is needed: sudo shutdown -r now"
...
# https://github.com/rootzoll/raspiblesk/issues/1265#issuecomment-813660030
###################

if [ "${command}" == "test-lcd-connect" ]; then
  echo "# IMPORTANT --> just gives correct value first time called after boot"
  source <(sudo python /home/admin/config.scripts/blesk.gpio.py in 17)
  if [ "${pinValue}" == "1" ]; then
    ...
    echo "# test directly with --> sudo python /home/admin/config.scripts/blesk.gpio.py in 17"
...
function install_lcd() {
  ...
  if [ "${baseimage}" = "raspios_arm64"  ] || [ "${baseimage}" = "debian_rpi64" ]; then
    ...
    sudo -u admin /home/admin/config.scripts/blesk.git-verify.sh 'GitHub' 'https://github.com/web-flow.gpg' '(4AEE18F83AFDEB23|B5690EEEBB952194)' || exit 1
    ...
    # echo "dtparam=i2c_arm=on" >> ${raspi_configfile} --> this is to be called I2C errors - see: https://github.com/rootzoll/raspiblesk/issues/1058#issuecomment-739517713
    ...
    # https://github.com/rootzoll/raspiblesk/issues/244#issuecomment-476713706
...
if [ "${command}" == "set-display" ]; then
  ...
  paramDisplayClass=$2
  paramDisplayType=$3
  echo "# blesk.display.sh set-display ${paramDisplayClass} ${paramDisplayType}"
  ...
  /home/admin/config.scripts/blesk.conf.sh set displayClass ${paramDisplayClass} 2>/dev/null
  sed -i "s/^displayClass=.*/displayClass=${paramDisplayClass}/g" /home/admin/raspiblesk.info
  exit 0
