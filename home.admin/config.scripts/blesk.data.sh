...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
    ...
    >&2 echo "# blesk.data.sh status [-inspect] # auto detect the old/best drives to use for storage, system and data"
    >&2 echo "# blesk.data.sh mount # mounts all drives and link all data folders"
    >&2 echo "# blesk.data.sh link # (re)create all symlinks to files and folders (also mapping old layout to new layout)"
    >&2 echo "# blesk.data.sh setup STOARGE [device] combinedData=[0|1] addSystemPartition=[0|1]"
    >&2 echo "# blesk.data.sh setup SYSTEM [device]"
    >&2 echo "# blesk.data.sh setup DATA [device]"
    >&2 echo "# blesk.data.sh clean STOARGE [device] combinedData=[0|1]"
    >&2 echo "# blesk.data.sh clean SYSTEM [device]"
    >&2 echo "# blesk.data.sh clean DATA [device]"
    >&2 echo "# blesk.data.sh copy-system [device] [system|storage]"
    >&2 echo "# blesk.data.sh recover STOARGE [device] combinedData=[0|1] bootFromStorage=[0|1]"
    >&2 echo "# blesk.data.sh recover SYSTEM [device]"
    >&2 echo "# blesk.data.sh recover DATA [device]"
    >&2 echo "# blesk.data.sh kill-boot [device] # deactivate boot function from install medium"
    >&2 echo "# blesk.data.sh migration [umbrel|citadel|mynode] [partition] [-test] # will migrate partition to raspiblesk"
    >&2 echo "# blesk.data.sh migration hdd [menu-prepare|run]"
    >&2 echo "# blesk.data.sh uasp-fix [-info] # deactivates UASP for non supported USB HDD Adapters"
    >&2 echo "# blesk.data.sh swap on # creates and activates an 8GB swapfile in / (Debian 12 only)"
    >&2 echo "# blesk.data.sh swap off # deactivates and removes the swapfile"
    >&2 echo "# blesk.data.sh reset # deletes all data & partitions on the storage device"
    >&2 echo "# blesk.data.sh expand [partition] # expands the partition to use all available space"
...
# On a old RaspiBlesk setup INTSALL+SYSTEM would be the same on the sd card and DATA+STORAGE on the USB conncted HDD.
...
if [ "$action" = "status" ]; then
    ...
    while IFS= read -r line; do
        if [ -n "$line" ]; then
            ...
            if [ -d "${mountPath}/app-storage" ]; then
                ...
                # check blochain data
                storageBlockchainGB=$(du -s ${mountPath}/app-storage/glcoin/blocks 2>/dev/null| awk '{printf "%.0f", $1/(1024*1024)}')
                if [ "${storageBlockchainGB}" = "" ]; then
                        # check old location
                        storageBlockchainGB=$(du -s ${mountPath}/glcoin/blocks 2>/dev/null| awk '{printf "%.0f", $1/(1024*1024)}')
                fi
                if [ "${storageBlockchainGB}" = "" ]; then
            ...
            # Check SYSTEM DRIVE
            elif [ -d "${mountPath}/boot" ] && [ -d "${mountPath}/home/admin/raspiblesk" ] && [ ${size} -gt 7 ]; then
            ...
            # Check MIGRATION: UMBREL
                ...
                echo "#  - UMBREL data detected - use 'blesk.data.sh migration'"
                storageMigration="umbrel"

            # Check MIGRATION: CITADEL
            elif [ -f "${mountPath}/citadel/info.json" ]; then
                echo "#  - CITADEL data detected - use 'blesk.data.sh migration'"
                storageMigration="citadel"

            # Check MIGRATION: MYNODE
            elif [ -f "${mountPath}/mynode/glcoin/glcoin.conf" ]; then
                echo "#  - MYNODE data detected - use 'blesk.data.sh migration'"
            ...
            # Check: CONFIG FILE
            if [ -f "${mountPath}/raspiblesk.conf" ] || [ -f "${mountPath}/app-data/raspiblesk.conf" ]; then
                dataConfigFound=1
                echo "#    * found raspiblesk.conf"
            fi
            ...
            if [ "$dataInspectPartition" = "1" ]; then
                if [ "$userWantsInspect" = "0" ]; then
                ...
                elif [ ! -d "/var/cache/raspiblesk" ]; then
                    echo "#  - skipping data inspect - RAMDISK not found"
                else

                    echo "#  - RUN INSPECT -> RAMDISK: /var/cache/raspiblesk/hdd-inspect"
                    mkdir /var/cache/raspiblesk/hdd-inspect 2>/dev/null
                    dataInspectSuccess=1

                    # make copy of raspiblesk.conf to RAMDISK (try old and new path)
                    cp -a ${mountPath}/raspiblesk.conf /var/cache/raspiblesk/hdd-inspect/raspiblesk.conf 2>/dev/null
                    cp -a ${mountPath}/app-data/raspiblesk.conf /var/cache/raspiblesk/hdd-inspect/raspiblesk.conf 2>/dev/null
                    if [ -f "/var/cache/raspiblesk/hdd-inspect/raspiblesk.conf" ]; then
                        echo "#    * raspiblesk.conf copied to RAMDISK"
                    fi

                    # make copy of WIFI config to RAMDISK (if available)
                    cp -a ${mountPath}/app-data/wifi /var/cache/raspiblesk/hdd-inspect/ 2>/dev/null
                    ...
                    cp -a ${mountPath}/app-data/sshd /var/cache/raspiblesk/hdd-inspect 2>/dev/null
                    cp -a ${mountPath}/app-data/ssh-root /var/cache/raspiblesk/hdd-inspect 2>/dev/null
                    if [ -d "/var/cache/raspiblesk/hdd-inspect/sshd" ] || [ -d "/var/cache/raspiblesk/hdd-inspect/ssh-root" ]; then
                        echo "#    * SSH keys copied to RAMDISK"
                    fi

                    # make copy of raspiblesk.setup to RAMDISK (if available from former systemcopy)
                    cp -a ${mountPath}/app-data/raspiblesk.setup /var/cache/raspiblesk/hdd-inspect/raspiblesk.setup 2>/dev/null
                    if [ -f "/var/cache/raspiblesk/hdd-inspect/raspiblesk.setup" ]; then
                        echo "#    * raspiblesk.setup copied to RAMDISK"
                    fi
    ...
    # recover: drives there but unmounted & blitz config exists (check raspiblesk.conf with -inspect if its update)
...
if [ "$action" = "link" ]; then
    ...
    chown glcoin:glcoin ${storageMountedPath}/app-storage ${mainMountPoint}/app-storage
    ...
    chown glcoin:glcoin ${dataMountedPath}/app-data ${mainMountPoint}/app-data
    mkdir -p /mnt/hdd/app-data/glcoin/wallets 2>/dev/null
    chown -R glcoin:glcoin ${dataMountedPath}/app-data/glcoin
    ...
    chown glcoin:glcoin ${mainMountPoint}/temp
    chmod 777 ${storageMountedPath}/temp
    chmod 777 ${mainMountPoint}/temp

    # /mnt/hdd/glcoin directory (move old data if needed & link for backwards compatibility)
    mkdir -p "${storageMountedPath}/app-storage/glcoin"
    mkdir -p "${dataMountedPath}/app-data/glcoin"
    chown glcoin:glcoin "${dataMountedPath}/app-data/glcoin"
    if [ -d "${storageMountedPath}/glcoin" ]; then
        # if ${storageMountedPath}/app-storage/glcoin is not a directory - delete & create directory
        if [ ! -d "${storageMountedPath}/app-storage/glcoin" ]; then
            echo "# fixing non-directory ${storageMountedPath}/app-storage/glcoin"
            rm -f "${storageMountedPath}/app-storage/glcoin"
            mkdir -p "${storageMountedPath}/app-storage/glcoin"
            chown glcoin:glcoin "${storageMountedPath}/app-storage/glcoin"
        fi
        # now move data
        /home/admin/_cache.sh set message "hdd-migrate"
        echo "# moving old data from ${storageMountedPath}/glcoin to ${storageMountedPath}/app-storage/glcoin"
        mvError=0

        # use mv to move main files first
        mv --force "${storageMountedPath}"/glcoin/* "${storageMountedPath}/app-storage/glcoin/" 2>/dev/null || mvError=1
        if [ $? -ne 0 ]; then
            echo "error='failed to mv ${storageMountedPath}/glcoin/* to ${storageMountedPath}/app-storage/glcoin/'"
            mvError=1
        fi

        # use rsync to move remaining files (hidden & merge directories)
        rsync -a --remove-source-files "${storageMountedPath}/glcoin/" "${storageMountedPath}/app-storage/glcoin/"
        if [ $? -ne 0 ]; then
            echo "error='failed to rsync ${storageMountedPath}/glcoin/ to ${storageMountedPath}/app-storage/glcoin/'"
            mvError=1
        fi
        
        # clean up
        if [ ${mvError} -eq 0 ] && [ "$(ls -A "${storageMountedPath}/glcoin" 2>/dev/null)" ]; then
            find "${storageMountedPath}/glcoin" -type d -empty -delete 2>/dev/null
            rm -rf "${storageMountedPath}/glcoin" 2>/dev/null
        fi
    fi
    if [ -d "${storageMountedPath}/app-storage/glcoin/wallet.dat" ]; then
        echo "# moving old wallet from ${storageMountedPath}/app-storage/glcoin/wallet.dat to ${dataMountedPath}/app-data/glcoin/wallets/wallet.dat"
        mv --force ${storageMountedPath}/app-storage/glcoin/wallet.dat ${dataMountedPath}/app-data/glcoin/wallets/wallet.dat
        if [ $? -ne 0 ]; then
            echo "error='failed to move ${storageMountedPath}/app-storage/glcoin/wallet.dat to ${dataMountedPath}/app-data/glcoin/wallets/wallet.dat'"
        fi
    fi
    if [ -f "${storageMountedPath}/app-storage/glcoin/glcoin.conf" ] && [ ! -L "${storageMountedPath}/app-storage/glcoin/glcoin.conf" ]; then
        if [ ! -f "${dataMountedPath}/app-data/glcoin/glcoin.conf" ]; then
            echo "# moving glcoin config file from ${storageMountedPath}/app-storage/glcoin to ${dataMountedPath}/app-data/glcoin"
            mv --force ${storageMountedPath}/app-storage/glcoin/glcoin.conf ${dataMountedPath}/app-data/glcoin/glcoin.conf
            if [ $? -ne 0 ]; then
                echo "error='failed to move ${storageMountedPath}/app-storage/glcoin/glcoin.conf to ${dataMountedPath}/app-data/glcoin/glcoin.conf'"
            fi
        else
            echo "error='there is ${storageMountedPath}/app-storage/glcoin/glcoin.conf AND ${dataMountedPath}/app-data/glcoin/glcoin.conf'"
        fi
    fi
    unlink ${mainMountPoint}/app-storage/glcoin/glcoin.conf 2>/dev/null

    echo "# For backwards compatibility: Liniking ${mainMountPoint}/glcoin"
    unlink ${mainMountPoint}/glcoin 2>/dev/null
    ln -s ${storageMountedPath}/app-storage/glcoin ${mainMountPoint}/glcoin
    chown glcoin:glcoin ${mainMountPoint}/glcoin
    chmod 777 ${mainMountPoint}/glcoin

    # /mnt/hdd/lnd directory (move old data if needed & link for backwards compatibility)
    mkdir -p "${dataMountedPath}/app-data/lnd"
    chown glcoin:glcoin "${dataMountedPath}/app-data/lnd"
    ...
    chown glcoin:glcoin ${mainMountPoint}/lnd
    ...
    # /mnt/hdd/aspiblesk.conf (move old file if needed & link for backwards compatibility)
    if [ -f "${storageMountedPath}/raspiblesk.conf" ] && [ ! -L "${storageMountedPath}/raspiblesk.conf" ]; then
        echo "# moving old config from ${storageMountedPath}/raspiblesk.conf to ${dataMountedPath}/app-data/raspiblesk.conf"
        mv --force ${storageMountedPath}/raspiblesk.conf ${dataMountedPath}/app-data/raspiblesk.conf
    fi
    if [ -f "${mainMountPoint}/raspiblesk.conf" ] && [ ! -L "${mainMountPoint}/raspiblesk.conf" ]; then
        mv --force ${mainMountPoint}/raspiblesk.conf ${mainMountPoint}/app-data/raspiblesk.conf
    fi

    echo "you find glcoin.conf now in ${mainMountPoint}/app-data/glcoin/glcoin.conf" > ${mainMountPoint}/glcoin/glcoin-conf.info
    ...
    ### glcoin user symbol links
    echo "# glcoin user symbol link: /home/glcoin/.glcoin"
    unlink /home/glcoin/.glcoin 2>/dev/null
    ...
    echo "# admin user symbol link: /home/admin/.glcoin"
...
if [ "$action" = "copy-system" ]; then
    ...
    echo "STARTED blesk.data.sh ${action} (${actionDevice})..." >> ${logFile}
    ...
    if [ "${computerType}" = "raspberrypi" ]; then
        ...
        echo "boot" > /var/cache/raspiblesk/temp/progress.txt
        rsync -axHAX --delete --info=progress2 ${bootPath} /mnt/disk_boot/ 2>&1 | stdbuf -oL tr '\r' '\n' | grep --line-buffered '%' | stdbuf -oL sed -n 's/.* \([0-9]\+\)% .*/\1%/p' >> /var/cache/raspiblesk/temp/progress.txt
        if [ $? -ne 0 ]; then
    ...
    echo "system" > /var/cache/raspiblesk/temp/progress.txt
    rsync -axHAX --delete\
        --exclude=/dev/* \
        --exclude=/proc/* \
        --exclude=/sys/* \
...
EOF
    ...
    rm /var/cache/raspiblesk/temp/progress.txt
...
if [ "$action" = "setup" ]; then
    ...
    echo "STARTED blesk.data.sh setup ..." >> ${logFile}
...
if [ "$action" = "recover" ] || [ "$action" = "clean" ]; then
    ...
    echo "STARTED blesk.data.sh ${action} ..." >> ${logFile}
    ...
    if [ "${action}" = "clean" ]; then
        ...
        if [ "${actionType}" = "DATA" ]; then
            ...
            /home/admin/config.scripts/blesk.data.sh setup DATA ${actionDevice}
            exit $?
        fi
        if [ "${actionType}" = "STORAGE" ]; then
            ...
            # 2. In app-storage alles löschen außer glcoin
            find /mnt/disk_storage/app-storage -mindepth 1 -maxdepth 1 \
                -not -name "glcoin" -exec rm -rf {} \;
            # 3. In glcoin alles löschen außer blocks und chainstate (beides behalten)
            if [ -d /mnt/disk_storage/app-storage/glcoin ]; then
              find /mnt/disk_storage/app-storage/glcoin -mindepth 1 -maxdepth 1 \
            ...
            ls -la /mnt/disk_storage/app-storage/glcoin >> ${logFile}
    ...
    echo "# DONE - blesk.data.sh ${action} ${actionType}" >> ${logFile}
    exit 0
fi
...
if [ "$1" = "migration" ] && [ "$2" = "hdd" ]; then
    ...
    if [ "${action}" = "run" ]; then
        ...
        echo "### MIGRATING HDD"
        echo "# see /var/cache/raspiblesk/temp/progress.txt for progress"
        ...
        echo "chain" > /var/cache/raspiblesk/temp/progress.txt
        ...
        # old layout: glcoin directory is still outside of app-storage
        if [ -d /mnt/migrate_source/glcoin ] && [ ! -L /mnt/migrate_source/glcoin ]; then
            echo "# rsync glcoin from source to target ..."
            mkdir -p /mnt/migrate_storage/app-storage/glcoin 2>/dev/null
            echo "glcoin" > /var/cache/raspiblesk/temp/progress.txt
            rsync -ah --info=progress2 /mnt/migrate_source/glcoin/ /mnt/migrate_storage/app-storage/glcoin/ 2>&1 | stdbuf -oL tr '\r' '\n' | grep --line-buffered '%' | stdbuf -oL sed -n 's/.* \([0-9]\+\)% .*/\1%/p' >> /var/cache/raspiblesk/temp/progress.txt
            if [ $? -ne 0 ]; then
                echo "error='failed to rsync glcoin'"
                exit 1
            fi
        fi
        ...
        echo "data" > /var/cache/raspiblesk/temp/progress.txt
        ...
        if [ -d /mnt/migrate_source/lnd ] && [ ! -L /mnt/migrate_source/lnd ]; then
            ...
            echo "lnd" > /var/cache/raspiblesk/temp/progress.txt
        ...
        if [ -d /mnt/migrate_source/tor ] && [ ! -L /mnt/migrate_source/tor ]; then
            ...
            echo "tor" > /var/cache/raspiblesk/temp/progress.txt
        ...
        # old layout: raspiblesk.conf file is still outside of app-data
        if [ -f /mnt/migrate_source/raspiblesk.conf ] && [ ! -L /mnt/migrate_source/raspiblesk.conf ]; then
            echo "# copy raspiblesk.conf from source to target ..."
            mkdir -p /mnt/migrate_data/app-data 2>/dev/null
            cp /mnt/migrate_source/raspiblesk.conf /mnt/migrate_data/app-data/raspiblesk.conf
            cp /mnt/migrate_source/.tmux.conf.local /mnt/migrate_data/app-data/.tmux.conf.local 2>/dev/null
            if [ $? -ne 0 ]; then
                echo "error='failed to rsync raspiblesk.conf'"
                exit 1
            fi
        else
            echo "# no old raspiblesk.conf file found"
        fi
        ...
        rm -rf /var/cache/raspiblesk/temp/progress.txt
...
if [ "$1" = "migration" ]; then
    ...
    echo "# blesk.data.sh migration" >> ${logFile}
    ...
    #       btcVersion=$(grep "lncm/glcoind" ${mountPath}/umbrel/app-data/glcoin/docker-compose.yml 2>/dev/null | sed 's/.*glcoind://' | sed 's/@.*//')
