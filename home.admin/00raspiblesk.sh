...
# or calls 'raspiblesk' on the terminal
#######################################
...
source <(/home/admin/config.scripts/blesk.copychain.sh status)
...
while [ ${exitMenuLoop} -eq 0 ]; do
  ...
  if [ "${state}" == "reboot" ] || [ "${state}" == "shutdown" ]; then
    ...
    clear
    echo "***********************************************************"
    echo "RaspiBlesk going to ${state}"
    echo "***********************************************************"
    ...
    if [ "${gotSCB}" == "1" ]; then
      ...
      if [ ${#error} -gt 0 ]; then
        ...
        if [ ${notMatchingSeed} -gt 0 ]; then
          ...
          echo "or the channel.backup file is from another RaspiBlesk."
          echo
        fi
...
echo
echo "***********************************"
echo "* RaspiBlesk Commandline"
echo "* Here be dragons .. have fun :)"
echo "***********************************"
if [ "${setupPhase}" == "done" ]; then
  echo "Glcoin command line options: ${network}-cli help"
